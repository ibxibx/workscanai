...
export default function ResultsPage() {
 ...
 if (loading) return (
  <div className="min-h-screen bg-white text-[#1d1d1f] pt-[88px] pb-[60px]">
    ...
    <div className="max-w-[980px] mx-auto px-6 text-center pt-[80px]">
      ...
      <div className="text-[20px] font-semibold text-[#1d1d1f] mb-[8px]">{t('loadingTitle')}</div>
      <div className="text-[14px] text-[#86868b]">
        {warming ? t('warmingLong') : t('warmingShort')}
      </div>
    </div>
  </div>
 )
 if (error || !analysisData) return (
   <div className="min-h-screen bg-white text-[#1d1d1f] pt-[88px] pb-[60px]">
     <div className="max-w-[980px] mx-auto px-6 text-center pt-[40px]">
       <div className="text-[24px] text-red-600 mb-3">{error || t('noData')}</div>
 ...
 const contextLabel = context === 'company' ? t('ctxCompany') : context === 'team' ? t('ctxTeam') : t('ctxPersonal')
 ...
 } catch { alert(tr('genFail', { fmt: fmt.toUpperCase() })) }
 }
 ...
 <p className="text-[14px] text-[#86868b]">{t('analysisId', { id, date: new Date().toLocaleDateString(locale === 'de' ? 'de-DE' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) })}</p>
 </div>
 ...
 <p className="text-[11px] font-semibold text-[#86868b] uppercase tracking-widest mb-[16px]">{tr('exportShare')}</p>
 ...
 <Download className="h-[15px] w-[15px] shrink-0" /> {tr('dlDocx')}
 </button>
 ...
 <Download className="h-[15px] w-[15px] shrink-0" /> {tr('dlPdf')}
 </button>
 ...
 <Download className="h-[15px] w-[15px] shrink-0" /> {t('n8nCanvasBtn')}
 </button>
 ...
 {copied ? <><Check className="h-[15px] w-[15px] text-green-600 shrink-0" /><span className="text-green-600">{tr('linkCopied')}</span></> : <><Share2 className="h-[15px] w-[15px] shrink-0" /><span>{tr('shareReport')}</span></>}
 </button>
 ...
 <Map className="h-[15px] w-[15px] shrink-0" /> {t('viewRoadmap')}
 </Link>
 </div>
 </div>
