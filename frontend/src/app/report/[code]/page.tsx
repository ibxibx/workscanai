...
interface AnalysisData {
  ...
  workflow: { id: number; share_code?: string; name: string; description: string; tasks: WorkflowTask[] }
...
async function getAnalysisByCode(code: string): Promise<AnalysisData | null> {
  try {
    const res = await fetch(`${API_BASE}/api/share/${code}`, { next: { revalidate: 3600 } })
...
export async function generateMetadata({ params }: { params: { code: string } }): Promise<Metadata> {
  const data = await getAnalysisByCode(params.code)
  ...
  const shareUrl = `https://workscanai.vercel.app/report/${params.code}`
  return {
  ...
  return (
    <div className="min-h-screen bg-white text-[#1d1d1f]">
      ...
      <div className="bg-[#0071e3] text-white text-center py-[10px] px-4 text-[13px]">
        ...
        <span className="opacity-50 ml-3 text-[11px]">#{params.code}</span>
      </div>
      ...
      <div className="max-w-[980px] mx-auto px-6 pt-[48px] pb-[80px]">
        ...
        <div className="mt-[48px] text-center text-[13px] text-[#86868b]">
          <p>Generated by <a href="https://workscanai.vercel.app" className="text-[#0071e3] hover:underline">WorkScanAI</a> · Report <span className="font-mono">#{params.code}</span></p>
        </div>
      </div>
    </div>
  )
