...
export default async function Image({ params }: { params: { code: string } }) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/share/${params.code}`)
    if (res.ok) {
  ...
  return new ImageResponse(
    (
      <div style={{
        width: '1200px', height: '630px', background: '#1d1d1f',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{
              width: '40px', height: '40px', background: '#0071e3', borderRadius: '10px',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ color: 'white', fontSize: '52px', fontWeight: 700, lineHeight: 1.1, maxWidth: '800px', fontStyle: 'italic' }}>
            {workflowName}
          </div>
          <div style={{ color: '#555', fontSize: '14px', fontFamily: 'monospace' }}>#{params.code}</div>
        </div>
