...
// self-gates (returns null when it doesn't apply).
...
export function RecommendationBlocks({ text }: { text: string }) {
  if (!text) return null
...
// ── Countdown badge ──
const COUNTDOWN_MAP: Record<string, { label: string; color: string; dot: string }> = {
    ...
    <div className="bg-white border border-[#e8e8ed] rounded-[20px] p-[20px] sm:p-[40px] mb-[24px] shadow-sm">
      ...
      <div className="space-y-[16px]">
        {results.map((result, index) => {
          ...
          return (
            <div key={index} className="border border-[#e8e8ed] rounded-[14px] p-[16px] sm:p-[28px] bg-[#fafafa]">
...
export function IndividualSections({ data, context }: { data: ReportData; context: ReportContext }) {
  ...
  // Aggregate pivot skills/roles across tasks
    ...
    if (r.pivot_skills) {
      try {
        ...
        if (Array.isArray(skills)) skills.forEach((s: unknown) => {
          const label = typeof s === 'string' ? s : (s as { skill?: string })?.skill ?? String(s)
  ...
  const skills = [...new Set(allSkills)].slice(0, 6)

  const seen = new Set<string>()
  type PivotRole = { role: string; risk: string; pivot_distance: string; automation_score_pct?: number }
  const allRoles: PivotRole[] = []
  data.results.forEach(r => {
    if (r.pivot_roles) {
      try {
        const roles = JSON.parse(r.pivot_roles)
        if (Array.isArray(roles)) roles.forEach((role: PivotRole) => {
          if (role.role && !seen.has(role.role) && allRoles.length < 8) { seen.add(role.role); allRoles.push(role) }
        })
      } catch { /* ignore */ }
    }
  })
                      ...
                      <div key={i} className="bg-[#fafafa] border border-[#e8e8ed] rounded-[10px] px-[14px] py-[12px]">
                        <div className="flex items-center justify-between gap-[8px] flex-wrap">
                          ...
                          <div className="flex items-center gap-[6px] shrink-0 flex-wrap">
                            {score != null && <span className={`text-[11px] font-bold px-[8px] py-[3px] rounded-full border ${scoreBg} ${scoreColor}`}>{score}% auto risk</span>}
                            <span className={`text-[10px] font-bold px-[8px] py-[3px] rounded-full border ${role.risk === 'low' ? 'bg-green-50 border-green-200 text-green-700' : role.risk === 'medium' ? 'bg-yellow-50 border-yellow-200 text-yellow-700' : 'bg-red-50 border-red-200 text-red-700'}`}>{role.risk} risk</span>
                            <span className="text-[10px] font-bold px-[8px] py-[3px] rounded-full border bg-blue-50 border-blue-200 text-blue-700">{role.pivot_distance} pivot</span>
                          </div>
                        </div>
                      </div>
                    )
    ...
    <>
      ...
      <div className="bg-white border border-[#e8e8ed] rounded-[20px] p-[20px] sm:p-[40px] mb-[24px] shadow-sm">
        ...
        <div className="mt-[28px] space-y-[12px]">
          {data.results.filter(r => r.difficulty === 'easy').sort((a, b) => b.ai_readiness_score - a.ai_readiness_score).slice(0, 5).map((r, i) => (
            <div key={i} className="flex items-start gap-[14px] bg-[#fafafa] border border-[#e8e8ed] rounded-[12px] p-[18px]">
              <div className="w-[28px] h-[28px] rounded-full bg-emerald-600 text-white text-[12px] font-bold flex items-center justify-center shrink-0 mt-[1px]">{i + 1}</div>
...
export function ContextSections({ data, context, workflowName }: { data: ReportData; context: ReportContext; workflowName: string }) {
  return (
    <>
      <IndividualSections data={data} context={context} />
      <TeamSections data={data} context={context} />
      <CompanySections data={data} context={context} workflowName={workflowName} />
      <AIReadinessSection data={data} context={context} />
    </>
  )
}
